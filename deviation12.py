...
<style>
    ...
    /* Tableau des déviations par tranche */
    .depth-table {
        width: 100%;
    ...
    /* Points de marqueurs sur la trajectoire 3D */
    .depth-marker {
        font-weight: 600;
        color: #3563E9;
        margin-right: 0.5rem;
...
# Fonction pour prédire la déviation à différentes profondeurs
def predict_deviation_by_depth(model_azimuth, model_inclinaison, initial_data, max_depth, step=30):
    """
    Prédit la déviation à différentes profondeurs le long du forage.
    
    Args:
        model_azimuth: Modèle entraîné pour la prédiction de la déviation d'azimuth
        model_inclinaison: Modèle entraîné pour la prédiction de la déviation d'inclinaison
        initial_data: DataFrame avec les paramètres initiaux du forage
        max_depth: Profondeur finale du forage
        step: Intervalle de profondeur pour les prédictions
    
    Returns:
        DataFrame avec les prédictions à chaque profondeur
    """
    depths = list(range(step, int(max_depth) + 1, step))
    if depths[-1] != max_depth:  # Ajouter la profondeur finale si elle n'est pas déjà incluse
        depths.append(max_depth)
    
    results = []
    
    # Récupérer les paramètres initiaux du forage
    azimuth_initial = initial_data['azimuth_initial'][0]
    inclinaison_initiale = initial_data['inclinaison_initiale'][0]
    lithologie = initial_data['lithologie'][0]
    company = initial_data['company'][0]
    vitesse_rotation = initial_data['vitesse_rotation'][0]
    
    # Paramètres cumulatifs
    azimuth_current = azimuth_initial
    inclinaison_current = inclinaison_initiale
    
    # Approche corrective: feedback des déviations précédentes
    for depth in depths:
        # Créer un DataFrame pour la profondeur actuelle
        data = pd.DataFrame({
            'profondeur_finale': [depth],
            'azimuth_initial': [azimuth_current],  # On utilise l'azimuth courant comme azimuth initial
            'inclinaison_initiale': [inclinaison_current],  # On utilise l'inclinaison courante comme inclinaison initiale
            'lithologie': [lithologie],
            'company': [company],
            'vitesse_rotation': [vitesse_rotation]
        })
        
        # Prédire les déviations pour cette tranche de profondeur
        predicted_azimuth = model_azimuth.predict(data)[0]
        predicted_inclinaison = model_inclinaison.predict(data)[0]
        
        # Mettre à jour les valeurs courantes pour la prochaine tranche
        azimuth_current += predicted_azimuth
        azimuth_current = azimuth_current % 360  # Normalisation à 360°
        
        inclinaison_current += predicted_inclinaison
        inclinaison_current = max(-90, min(0, inclinaison_current))  # Contraindre entre -90 et 0
        
        # Stocker les résultats
        results.append({
            'profondeur': depth,
            'deviation_azimuth': predicted_azimuth,
            'deviation_inclinaison': predicted_inclinaison,
            'azimuth_cumul': azimuth_current,
            'inclinaison_cumul': inclinaison_current
        })
    
    return pd.DataFrame(results)

# Fonction pour générer le graphique 3D avec points à différentes profondeurs
def generate_advanced_3d_trajectory(initial_azimuth, initial_inclination, depth_predictions, max_depth):
    """
    Génère une visualisation 3D de la trajectoire avec des points à chaque profondeur.
    
    Args:
        initial_azimuth: Azimuth initial du forage
        initial_inclination: Inclinaison initiale du forage
        depth_predictions: DataFrame avec les prédictions par profondeur
        max_depth: Profondeur finale du forage
        
    Returns:
        Figure Plotly 3D
    """
    # Fonction de conversion coordonnées polaires -> cartésiennes
    def sph_to_cart(depth, azimuth, inclination):
        ...
        z = depth * np.sin(inclination_rad)
    ...
    # Ajouter les points pour chaque profondeur prédite
    x_coords, y_coords, z_coords = [], [], []
    markers_x, markers_y, markers_z = [], [], []
    markers_text = []
    
    # Point de départ (surface)
    x, y, z = 0, 0, 0
    x_coords.append(x)
    y_coords.append(y)
    z_coords.append(z)
    
    # Ajouter les points des segments du forage
    prev_depth = 0
    for _, row in depth_predictions.iterrows():
        depth = row['profondeur']
        azimuth = row['azimuth_cumul']
        inclination = row['inclinaison_cumul']
        
        # Calculer les coordonnées pour cette profondeur
        x, y, z = sph_to_cart(depth, azimuth, inclination)
        
        # Ajouter aux listes pour la trajectoire
        x_coords.append(x)
        y_coords.append(y)
        z_coords.append(z)
        
        # Ajouter aux listes pour les marqueurs
        markers_x.append(x)
        markers_y.append(y)
        markers_z.append(z)
        markers_text.append(f"Profondeur: {depth}m<br>Azimuth: {azimuth:.2f}°<br>Inclinaison: {inclination:.2f}°")
        
        prev_depth = depth
    
    # Surface (grille)
    margin = 100  # Marge autour du forage pour la surface
    x_min, x_max = min(x_coords) - margin, max(x_coords) + margin
    y_min, y_max = min(y_coords) - margin, max(y_coords) + margin
    
    x_surface = np.linspace(x_min, x_max, 10)
    y_surface = np.linspace(y_min, y_max, 10)
    ...
    # Ajouter les marqueurs de profondeur
    fig.add_trace(go.Scatter3d(
        x=markers_x,
        y=markers_y,
        z=markers_z,
    ...
    x_ideal, y_ideal, z_ideal = sph_to_cart(max_depth, initial_azimuth, initial_inclination)
    ...
    fig.update_layout(
        title=dict(
            text=f"Trajectoire du forage (Écart final: {final_deviation:.2f} m)",
            font=dict(family="Poppins", size=20)
        ),
        scene=dict(
            xaxis_title='Est (m)',
            yaxis_title='Nord (m)',
...
with st.sidebar:
    ...
    # Options pour la prédiction par tranches
    st.markdown('<p class="sidebar-heading" style="color: white;">Prédiction par tranche</p>', unsafe_allow_html=True)
    depth_interval = st.number_input("Intervalle (m)", min_value=10, max_value=100, value=30, step=5, 
                                    help="Intervalle de profondeur pour les prédictions intermédiaires")
...
if df is not None:
    ...
    with tabs[2]:  # Prédiction
        ...
        if predict_button and st.session_state.model_trained:
            ...
            # Prédire les déviations finales
            ...
            with col1:
                st.metric("Déviation d'azimuth totale", f"{predicted_azimuth:.2f}°")
                st.metric("Azimuth final", f"{azimuth_final:.2f}°")
            
            with col2:
                st.metric("Déviation d'inclinaison totale", f"{predicted_inclinaison:.2f}°")
            ...
            # Nouveauté: Prédiction par tranche de profondeur
            st.markdown("<h3>Prédiction de déviation par tranches</h3>", unsafe_allow_html=True)
            
            # Faire les prédictions par tranche
            depth_predictions = predict_deviation_by_depth(
                st.session_state.model_azimuth, 
                st.session_state.model_inclinaison, 
                input_data, 
                prof_finale_input, 
                step=depth_interval
            )
            
            # Afficher les prédictions par tranche sous forme de tableau
            st.markdown("<h4>Déviations prédites tous les {} mètres</h4>".format(depth_interval), unsafe_allow_html=True)
            
            # Créer un tableau HTML pour les prédictions par profondeur
            st.markdown("""
            <div style="overflow-x: auto;">
                <table class="depth-table">
                    <thead>
                        <tr>
                            <th>Profondeur (m)</th>
                            <th>Déviation d'azimuth (°)</th>
                            <th>Déviation d'inclinaison (°)</th>
                            <th>Azimuth cumulé (°)</th>
                            <th>Inclinaison cumulée (°)</th>
                        </tr>
                    </thead>
                    <tbody>
            """, unsafe_allow_html=True)
            ...
            for _, row in depth_predictions.iterrows():
                table_rows += f"""
                <tr>
                    <td><span class="depth-marker">•</span>{row['profondeur']:.1f}</td>
                    <td>{row['deviation_azimuth']:.2f}°</td>
                    <td>{row['deviation_inclinaison']:.2f}°</td>
                    <td>{row['azimuth_cumul']:.2f}°</td>
                    <td>{row['inclinaison_cumul']:.2f}°</td>
                </tr>
                """
            
            st.markdown(table_rows + """
                    </tbody>
                </table>
            </div>
            """, unsafe_allow_html=True)
            
            # Graphiques des déviations par profondeur
            st.markdown("<h4>Évolution des déviations avec la profondeur</h4>", unsafe_allow_html=True)
            
            col1, col2 = st.columns(2)
            
            with col1:
                # Graphique de l'évolution de l'azimuth avec la profondeur
                fig_az_depth = px.line(
                    depth_predictions, 
                    x='profondeur', 
                    y=['azimuth_cumul'], 
                    title="Évolution de l'azimuth avec la profondeur",
                    template="plotly_white",
                    labels={"profondeur": "Profondeur (m)", "value": "Azimuth (°)", "variable": "Paramètre"}
                )
                fig_az_depth.update_traces(mode='markers+lines', marker=dict(size=8))
                fig_az_depth.update_layout(
                    hovermode='x unified',
                    legend=dict(
                        orientation="h",
                        yanchor="bottom",
                ...
                fig_az_depth.data[0].name = "Azimuth"
                
                # Ajout d'une ligne horizontale pour l'azimuth initial
                fig_az_depth.add_hline(
                    y=azimuth_initial_input,
                    line_dash="dash",
                    line_color="gray",
                    annotation_text="Azimuth initial",
                    annotation_position="bottom right"
                )
                
                st.plotly_chart(fig_az_depth, use_container_width=True)
            
            with col2:
                # Graphique de l'évolution de l'inclinaison avec la profondeur
                fig_inc_depth = px.line(
                    depth_predictions, 
                    x='profondeur', 
                    y=['inclinaison_cumul'], 
                    title="Évolution de l'inclinaison avec la profondeur",
                    template="plotly_white",
                    labels={"profondeur": "Profondeur (m)", "value": "Inclinaison (°)", "variable": "Paramètre"}
                )
                fig_inc_depth.update_traces(mode='markers+lines', marker=dict(size=8))
                fig_inc_depth.update_layout(
                    hovermode='x unified',
                    legend=dict(
                        orientation="h",
                        yanchor="bottom",
                ...
                # Personnalisation des légendes
                fig_inc_depth.data[0].name = "Inclinaison"
                
                # Ajout d'une ligne horizontale pour l'inclinaison initiale
                fig_inc_depth.add_hline(
                    y=inclinaison_initiale_input,
                    line_dash="dash",
                    line_color="gray",
                    annotation_text="Inclinaison initiale",
                    annotation_position="bottom right"
                )
                
                st.plotly_chart(fig_inc_depth, use_container_width=True)
            
            # Visualisation 3D de la trajectoire avec points à différentes profondeurs
            st.markdown("<h3>Visualisation 3D de la trajectoire</h3>", unsafe_allow_html=True)
            
            # Générer la visualisation 3D avec points à différentes profondeurs
            fig_3d, final_deviation = generate_advanced_3d_trajectory(
                azimuth_initial_input,
                inclinaison_initiale_input,
                depth_predictions,
                prof_finale_input
            )
            ...
            if deviation_magnitude < 5:
                ...
                - Surveiller régulièrement l'orientation pendant l'opération, en particulier aux alentours de {} mètres où la déviation s'accélère.
                """
                gauge_color = "#0CCE6B"
            elif deviation_magnitude < 15:
                recommendations = """
                ...
                - Prévoir des mesures de contrôle plus fréquentes pendant le forage, en particulier aux profondeurs de {} mètres.
                ...
                - Utiliser des stabilisateurs supplémentaires pour maintenir la trajectoire, particulièrement à partir de {} mètres.
                - Envisager des techniques de forage dirigé si disponibles.
                - Effectuer des mesures de contrôle très fréquentes.
                """
                gauge_color = "#FF4B4B"
            
            # Déterminer les points critiques de déviation
            # Calculer les deltas entre chaque intervalle pour identifier les plus grands changements
            depth_predictions['delta_az'] = depth_predictions['deviation_azimuth'].abs()
            depth_predictions['delta_inc'] = depth_predictions['deviation_inclinaison'].abs()
            depth_predictions['delta_combined'] = (depth_predictions['delta_az']**2 + depth_predictions['delta_inc']**2)**0.5
            
            # Trouver les 2 intervalles de profondeur avec les plus grandes déviations
            critical_points = depth_predictions.sort_values(by='delta_combined', ascending=False).head(2)
            critical_depths = critical_points['profondeur'].tolist()
            
            # Formater les profondeurs critiques pour les recommandations
            critical_depths_str = f"{critical_depths[0]:.0f}"
            if len(critical_depths) > 1:
                critical_depths_str += f" et {critical_depths[1]:.0f}"
            
            # Insérer les profondeurs critiques dans les recommandations
            recommendations = recommendations.format(critical_depths_str)
            ...
            # Créer des lignes pour le rapport des déviations par tranches
            depth_report_lines = []
            for _, row in depth_predictions.iterrows():
                depth_report_lines.append(
                    f"- Profondeur {row['profondeur']:.0f}m: "
                    f"Azimuth {row['azimuth_cumul']:.2f}° (déviation: {row['deviation_azimuth']:.2f}°), "
                    f"Inclinaison {row['inclinaison_cumul']:.2f}° (déviation: {row['deviation_inclinaison']:.2f}°)"
                )
            
            depth_report_text = "\n".join(depth_report_lines)
...
RÉSULTATS DE LA PRÉDICTION FINALE
------------------------------
- Déviation d'azimuth totale: {predicted_azimuth:.2f}°
- Déviation d'inclinaison totale: {predicted_inclinaison:.2f}°
...
ÉVOLUTION DE LA DÉVIATION PAR TRANCHES DE {depth_interval}m
-------------------------------------------------------
{depth_report_text}

POINTS CRITIQUES
-------------
Les déviations les plus importantes sont observées aux profondeurs: {critical_depths_str} mètres.
